...
/// `state` is stored MSB-first (state[0] is the oldest bit).
/// The polynomial `poly` has poly[0]=1, poly[i] is tap at position i.
pub fn lfsr_next(poly: &[u8], state: &[u8]) -> u8 {
    let l = poly.len() - 1;
    assert_eq!(state.len(), l);
    let mut out = 0u8;
    for i in 1..=l {
        out ^= poly[i] & state[i - 1];
    }
    out
}
...
mod tests {
    ...
    fn alternating_sequence() {
        // 1,0,1,0,1,0 has LFSR: C(x) = 1 + x (taps at 1), length 1...
        // Actually 1,0,1,0... is generated by x+1 (XOR previous), so length=1
        let s = vec![1u8, 0, 1, 0, 1, 0, 1, 0];
        let (poly, l) = berlekamp_massey(&s);
        assert_eq!(l, 1);
        // verify poly generates the sequence
        let mut state = vec![s[0]];
        for i in 1..s.len() {
